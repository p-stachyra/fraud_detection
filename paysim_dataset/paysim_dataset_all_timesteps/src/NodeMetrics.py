class NodeMetrics:
    ...
    def getHITSCentrality(self):
        query_hits = """
                            CALL gds.alpha.hits.stream('%s', {hitsIterations: 20})
                            YIELD nodeId, values
                            RETURN gds.util.asNode(nodeId).id AS id, values.auth AS authorityScore, values.hub as hubScore
                            ORDER BY id ASC
                            """ % self.graph_name

        return self.gds.run_cypher(query_hits)
